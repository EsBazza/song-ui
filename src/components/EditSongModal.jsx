...
import { X, Save, Link as LinkIcon, Music, User, Disc, Tag, Loader2 } from 'lucide-react';
import { updateSong } from '../api';

const EditSongModal = ({ isOpen, onClose, song, onSongUpdated }) => {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl sm:p-8">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-semibold tracking-tight text-white">Edit song</h2>
            <p className="mt-1 text-sm text-slate-500">Update details and save changes.</p>
          </div>
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-xl py-3 pl-10 pr-4 text-white focus:outline-none focus:border-slate-600 transition-all placeholder:text-slate-600"
            ...
            <div className="relative">
              <Disc className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={16} />
              <input
                name="album"
                placeholder="Album"
                className="w-full bg-slate-950 border border-slate-800 rounded-xl py-3 pl-10 pr-4 text-white focus:outline-none focus:border-slate-600 transition-all placeholder:text-slate-600"
